...
function _ordered_fold_stage_workspace_spec(stage; path::Tuple, name_prefix::Symbol)
    ...
    names = (
        values = Symbol(name_prefix, :_values), order = Symbol(name_prefix, :_order),
    ...
    state_leaves = map(eachindex(state_names)) do index
        ...
        _workspace_leaf(
            name, (path..., :state, state_names[index]),
            eltype(storage), size(storage); role = :ordered_fold_state
        )
    end
    leaves = (base_leaves..., state_leaves...)
    state_template = NamedTuple{state_names}(
        Tuple(
            map(eachindex(state_names)) do index
                _WorkspaceLeafSlot(
                    Symbol(
...
function _ordered_fold_stage_workspace_from_tree(tree, spec)
    return _OrderedFoldStageWorkspace(
        tree.values, tree.order, tree.status,
        tree.validation, tree.state, tree
    )
end

@inline function _ordered_fold_stage_fail!(
        run, code::Int32,
...
@generated function _stage_reads(
    ...
    reads = map(1:fieldcount(A)) do index
        :(
            _stage_read(
                stage, getfield(stage.accesses, $index), item,
                validation
            )
        )
    end
...
@generated function _ordered_fold_stage_initialize_item!(
    ...
    return Expr(:block, body..., :(nothing))
end
...
@kernel function _ordered_fold_stage_evaluate_kernel!(
    ...
    if item <= extent &&
        ...
        elseif enabled
            ...
            result = _call_stage_evaluator(
                qualified, Int32(item),
...
@inline function _ordered_fold_stage_validate_writes(
        writes::BoundedWrites{K}, extent,
        component::Int32
    ) where {K}
...
@generated function _ordered_fold_stage_validate_step!(
        state::_PreparedFoldState{C}, scratch,
        step::FoldStep
    ) where {C}
    names = C.parameters[1]
    checks = Any[]
...
@generated function _ordered_fold_stage_apply_step!(
        state::_PreparedFoldState{C}, scratch,
        step::FoldStep
    ) where {C}
    names = C.parameters[1]
            ...
            :(
                    begin
                        ...
                        _ordered_fold_stage_apply_writes!(
                            getfield(scratch, $(QuoteNode(names[i]))),
                            getfield(step.updates, $(QuoteNode(names[i])))
                        )
                    end
                ) for i in eachindex(names)
...
@inline function _ordered_fold_stage_less(order, values, left, right)
    ...
    comparison = _canonical_order_compare(
        _ordering_extract(order.key, @inbounds(values[left])),
...
@inline function _ordered_fold_stage_index_less(
        order, values,
...
@generated function _ordered_fold_stage_commit_item!(
    ...
    return Expr(:block, copies..., :(nothing))
end
...
@kernel function _ordered_fold_stage_finalize_kernel!(run)
    ...
    if _ordered_fold_stage_prefix_ok(run)
        ...
        if index == 1
            ...
            existing = @inbounds validation[
                _VALIDATION_FAILURE_CLASS,
                run.lease_index,
            ]
            if existing != UInt32(0)
                _store_program_validation_status!(
                    run.program_validation,
                    run.lease_index, existing,
                    @inbounds(
                        validation[
                            _VALIDATION_CONTEXT_INDEX,
                            run.lease_index,
                        ]
                    ),
                    @inbounds(
                        validation[
                            _VALIDATION_PRIMARY_RECORD,
                            run.lease_index,
                        ]
                    ),
                    @inbounds(
                        validation[
                            _VALIDATION_SECONDARY_RECORD,
                            run.lease_index,
                        ]
                    ),
                    @inbounds(
                        validation[
                            _VALIDATION_WITNESS_BITS,
                            run.lease_index,
                        ]
                    )
                )
            elseif code == Int32(_ORDERED_FOLD_DUPLICATE_ORDER)
                ...
                for position in Int32(2):run.source_count
                    ...
                    item == 0 && break
                    if previous != 0 && _ordered_fold_stage_equal(
                            run.order,
                            run.workspace.values, previous, item
                        )
                        _store_validation_status!(
                            validation, run.lease_index,
        ...
        # Initialization copies only private scratch; a closed gate must not
        # publish that initializer over the destination's retained value.
        if code == 0 && _stage_gate_open(
                run.stage.gate, run.stage,
                run.boundary_parameters
            )
            _ordered_fold_stage_commit_item!(
                run.state,
...
function _prepare_ordered_fold_stage(
        admission::_StageAdmission,
        workspace::_OrderedFoldStageWorkspace
    )
    stage = admission.stage
    only(stage.publications).law isa _PreparedOrderedFoldLaw || throw(
    ...
    state = only(only(stage.publications).components).state
    state_extent = maximum(
        (
            length(
                    _prepared_stage_field(
                        stage.fields,
                        component.target
                    )
    ...
    state_extent <= typemax(Int32) || throw(
        LocalMathValidationError(
    ...
    return _OrderedFoldStagePreparation(
        admission.backend, stage, workspace,
        workspace.validation, Int32(state_extent)
    )
end

function _execute_ordered_fold_stage!(
        prepared::_OrderedFoldStagePreparation,
        parameters::Tuple, lease_index::Int32, predecessors::Tuple,
        relation_guard, program_validation
    )
    qualified = _QualifiedEvaluation(
        _stage_evaluation(prepared.stage),
    ...
    boundary_parameters = (;
        prefix = qualified.parameters.prefix,
    ...
    finalization = (;
        ...
        stage = recurrence_stage, boundary_parameters, state,
